  ...
  : _world(world)
{
  ...
  if (this->_music->openFromFile(g_folder + "/Sound/menu.ogg"))
    {
...
void SFMLMenu::init()
{
  ...
  this->_buttonplay->addTexture(g_folder + "/Images/playgameButton.png", 250, 80);
  this->_buttonCredit = new SFMLButton(this->_window, 540, 490, 250, 100);
  this->_buttonCredit->addTexture(g_folder + "/Images/creditsButton.png", 338, 94);

  this->_buttonSolo = new SFMLButton(this->_window, 25, 200, 200, 75);
  this->_buttonSolo->addTexture(g_folder + "/Images/buttonSolo.png", 171, 48);
  ...
  if (texture->loadFromFile(g_folder + "/Images/menuBackground.jpg"))
    {
  ...
  if (logo->loadFromFile(g_folder + "/Images/logo.png"))
    {
